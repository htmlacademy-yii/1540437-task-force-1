...
CREATE TABLE IF NOT EXISTS `taskforce`.`users` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `citiy_id` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `taskforce`.`tasks` (
  ...
  `created_at` TIMESTAMP NOT NULL DEFAULT current_timestamp(),
  `updated_at` TIMESTAMP NULL DEFAULT NULL ON UPDATE current_timestamp,
  `done_before` DATE NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `taskforce`.`task_location` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `task_id` INT(10) UNSIGNED NOT NULL,
  `lattitude` FLOAT(11) NULL DEFAULT NULL,
  `longtitude` FLOAT(11) NULL DEFAULT NULL,
  ...
  INDEX `fk_task_location_tasks_idx` (`task_id` ASC),
  CONSTRAINT `fk_task_location_tasks`
    FOREIGN KEY (`task_id`)
    REFERENCES `taskforce`.`tasks` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
